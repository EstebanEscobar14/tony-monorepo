...
import { ThemeService, type Theme, TonCommandPaletteService, type Command } from '@tony-ui/core';
      ...
      <div class="flex h-full items-center justify-between px-4 lg:px-6">
        ...
        <div class="flex items-center gap-2 sm:gap-4 min-w-0">
          ...
          <a [routerLink]="i18n.localizeLink('/')" class="flex items-center gap-2 text-lg font-bold tracking-tight shrink-0">
            <docs-brand-logo [size]="34" label="Capital Flow" />
          </a>
...
export class NavbarComponent {
  ...
  private readonly commandPalette = inject(TonCommandPaletteService);
  ...
  readonly commands = computed<Command[]>(() => {
    ...
    // Theme commands
    commands.push(
      { id: 'theme-light', label: 'Light Theme', group: 'Theme', icon: 'sun', action: () => this.themeService.setTheme('light') },
      { id: 'theme-dark', label: 'Dark Theme', group: 'Theme', icon: 'moon', action: () => this.themeService.setTheme('dark') },
      { id: 'theme-corporate', label: 'Corporate Theme', group: 'Theme', icon: 'building', action: () => this.themeService.setTheme('corporate') },
    );

    // Language
    commands.push({
      id: 'lang-switch',
      label: this.i18n.locale() === 'es' ? 'Switch to English' : 'Cambiar a Espanol',
      group: 'Settings',
      icon: 'globe',
    ...
    commands.push({
      ...
      label: 'Open GitHub',
      group: 'Actions',
      action: () => window.open('https://github.com/coci-dev/tony-ui', '_blank'),
    });
