        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
          @for (token of tokens; track token) {
            <div class="flex items-center gap-2 rounded-sm border border-border p-2">
              <div class="h-6 w-6 rounded-sm border border-border" [style.background-color]="'var(--ton-' + token + ')'"></div>
...
export class ThemingOverviewComponent {
  ...
  variablesCode = `:root {
  --ton-background: #ffffff;
  --ton-foreground: #0a0a0a;
  --ton-primary: #171717;
  --ton-primary-foreground: #fafafa;
  --ton-secondary: #f5f5f5;
  --ton-secondary-foreground: #171717;
  --ton-muted: #f5f5f5;
  --ton-muted-foreground: #737373;
  /* ... */
}`;

  serviceCode = `import { ThemeService } from '@tony-ui/core';
