...
import { TonSwitchComponent, TonLabelDirective } from '@tony-ui/core';
...
@Component({
  ...
  standalone: true,
  imports: [CodeBlockComponent, ComponentPreviewComponent, PropsTableComponent, TonSwitchComponent, TonLabelDirective, ReactiveFormsModule],
  template: `
    <div class="space-y-8">
      <div>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="basicCode">
          <div class="flex items-center gap-3">
            <ton-switch [(checked)]="darkMode" />
            <label tonLabel>Dark Mode: {{ darkMode() ? 'ON' : 'OFF' }}</label>
          </div>
        ...
        <docs-component-preview [code]="sizesCode">
          <div class="flex items-center gap-6">
            <div class="flex items-center gap-2">
              <ton-switch size="sm" />
              <label tonLabel>Small</label>
            </div>
            <div class="flex items-center gap-2">
              <ton-switch size="md" />
              <label tonLabel>Medium</label>
            </div>
            <div class="flex items-center gap-2">
              <ton-switch size="lg" />
              <label tonLabel>Large</label>
            </div>
          </div>
        </docs-component-preview>
      </section>
        ...
        <docs-component-preview [code]="exampleCode" language="typescript">
          <div class="space-y-4 max-w-sm w-full">
            <div class="flex items-center justify-between">
              <label tonLabel>Notifications</label>
              <ton-switch [(checked)]="notifications" />
            </div>
            <div class="flex items-center justify-between">
              <label tonLabel>Marketing emails</label>
              <ton-switch [(checked)]="marketing" />
            </div>
            <div class="flex items-center justify-between">
              <label tonLabel>Auto-updates</label>
              <ton-switch [(checked)]="autoUpdates" />
            </div>
          </div>
        </docs-component-preview>
      </section>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="reactiveFormsCode" language="typescript">
          <div class="flex items-center gap-3">
            <ton-switch [formControl]="switchCtrl" />
            <label tonLabel>Value: {{ switchCtrl.value }}</label>
          </div>
        </docs-component-preview>
      </section>
...
<ton-switch [formControl]="switchCtrl" />`;
  ...
  importCode = `import { TonSwitchComponent } from '@tony-ui/core';`;
  basicCode = `<ton-switch [(checked)]="darkMode" />`;
  sizesCode = `<ton-switch size="sm" />
<ton-switch size="md" />
<ton-switch size="lg" />`;
