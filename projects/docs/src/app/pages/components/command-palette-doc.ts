...
import { TonCommandPaletteService, TonButtonDirective, ThemeService, type Command, type Theme } from '@tony-ui/core';
    ...
    <div class="space-y-8">
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="basicCode" language="markup">
          <button tonBtn (click)="openBasic()">Open Command Palette</button>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="fullCode" language="typescript">
          <div class="space-y-3 text-center">
            <button tonBtn (click)="openFull()">Open Full Palette</button>
...
export class CommandPaletteDocComponent {
  ...
  private readonly palette = inject(TonCommandPaletteService);
  ...
  fullCommands: Command[] = [
    ...
    { id: 'install', label: 'Installation Guide', group: 'Navigation', description: 'Getting started with TonyUI', action: () => { this.router.navigate(['/docs/getting-started/installation']); this.lastAction.set('Navigated → Installation'); } },
  ...
  importCode = `import { TonCommandPaletteService, type Command } from '@tony-ui/core';`;

  basicCode = `<button tonBtn (click)="openPalette()">Open Command Palette</button>`;

  fullCode = `import { Component, inject, HostListener, signal } from '@angular/core';
import { TonCommandPaletteService, type Command } from '@tony-ui/core';
...
export class MyComponent {
  private palette = inject(TonCommandPaletteService);
  readonly lastAction = signal('');
