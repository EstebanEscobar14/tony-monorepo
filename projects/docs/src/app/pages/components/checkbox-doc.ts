...
import { TonCheckboxDirective, TonLabelDirective } from '@tony-ui/core';

@Component({
  selector: 'docs-checkbox-doc',
  standalone: true,
  imports: [CodeBlockComponent, ComponentPreviewComponent, PropsTableComponent, TonCheckboxDirective, TonLabelDirective],
  template: `
    <div class="space-y-8">
      <div>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="basicCode">
          <div class="flex items-center gap-2">
            <input type="checkbox" tonCheckbox id="terms" />
            <label tonLabel for="terms">Accept terms and conditions</label>
          </div>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="sizesCode">
          <div class="flex items-center gap-6">
            <div class="flex items-center gap-2">
              <input type="checkbox" tonCheckbox size="sm" id="cb-sm" />
              <label tonLabel for="cb-sm">Small</label>
            </div>
            <div class="flex items-center gap-2">
              <input type="checkbox" tonCheckbox size="md" id="cb-md" />
              <label tonLabel for="cb-md">Medium</label>
            </div>
            <div class="flex items-center gap-2">
              <input type="checkbox" tonCheckbox size="lg" id="cb-lg" />
              <label tonLabel for="cb-lg">Large</label>
            </div>
          </div>
        </docs-component-preview>
      </section>
...
export class CheckboxDocComponent {
  ...
  importCode = `import { TonCheckboxDirective } from '@tony-ui/core';`;
  basicCode = `<div class="flex items-center gap-2">
  <input type="checkbox" tonCheckbox id="terms" />
  <label tonLabel for="terms">Accept terms</label>
</div>`;
  sizesCode = `<input type="checkbox" tonCheckbox size="sm" />
<input type="checkbox" tonCheckbox size="md" />
<input type="checkbox" tonCheckbox size="lg" />`;
  exampleCode = `todos = [
...
<input type="checkbox" tonCheckbox [checked]="todo.done()"
       (change)="todo.done.set(!todo.done())" />`;
