...
@Component({
  ...
  standalone: true,
  imports: [CodeBlockComponent, ComponentPreviewComponent, PropsTableComponent, TonButtonDirective],
  template: `
    <div class="space-y-8">
      <div>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="basicCode">
          <div class="flex gap-2 flex-wrap">
            <button tonBtn (click)="openSheet('right')">Open Right</button>
            <button tonBtn variant="outline" (click)="openSheet('left')">Open Left</button>
            <button tonBtn variant="outline" (click)="openSheet('top')">Open Top</button>
            <button tonBtn variant="outline" (click)="openSheet('bottom')">Open Bottom</button>
          </div>
      ...
      <section class="space-y-4">
        ...
        <h3 class="text-lg font-medium">{{ t().tonSheetService }}</h3>
        <docs-props-table [props]="serviceProps()" />
        <h3 class="text-lg font-medium mt-4">{{ t().tonSheetConfig }}</h3>
...
export class SheetDocComponent {
  ...
  private readonly sheetService = inject(TonSheetService);
  ...
  basicCode = `const sheetService = inject(TonSheetService);
...
sheetService.open(MySheetComponent, { side: 'right' });`;
  ...
  readonly serviceProps = computed<PropDef[]>(() => [
    { name: 'open(component, config?)', type: 'TonSheetRef<R>', default: '-', description: this.t().servicePropDescriptions.open },
...
@Component({
  ...
  template: `
    <div class="relative">
      <button tonSheetClose class="absolute right-0 top-0">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
      </button>
      <div tonSheetHeader>
        <h2 tonSheetTitle>Sheet from {{ data.side }}</h2>
        <p tonSheetDescription>This sheet slides from the {{ data.side }} side.</p>
      </div>
      <div class="py-6">
        <p class="text-sm text-muted-foreground">Content goes here.</p>
      </div>
      <button tonBtn (click)="dialogRef.close()">Close</button>
    </div>
  `,
})
class DemoSheetComponent {
  readonly dialogRef = inject(DialogRef);
  readonly data = inject<{ side: string }>(TON_SHEET_DATA);
}
...
@Component({
  ...
  template: `
    <div class="relative">
      ...
      <div tonSheetHeader>
        <h2 tonSheetTitle>Shopping Cart</h2>
        <p tonSheetDescription>{{ items.length }} items in your cart</p>
      </div>
      <div tonSheetContent>
        <div class="space-y-4">
      ...
      <div class="flex justify-end gap-2 pt-4">
        <button tonBtn variant="outline" (click)="dialogRef.close()">Continue Shopping</button>
        <button tonBtn (click)="dialogRef.close('checkout')">Checkout</button>
      </div>
    </div>
  `,
})
class CartSheetComponent {
  readonly dialogRef = inject(DialogRef);
  readonly items = inject<CartItem[]>(TON_SHEET_DATA);
}
