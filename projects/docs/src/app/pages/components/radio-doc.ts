...
import { TonRadioDirective, TonLabelDirective } from '@tony-ui/core';
...
@Component({
  ...
  standalone: true,
  imports: [CodeBlockComponent, ComponentPreviewComponent, PropsTableComponent, TonRadioDirective, TonLabelDirective],
  template: `
    <div class="space-y-8">
      <div>
          ...
          <div class="space-y-3">
            @for (opt of paymentOptions; track opt.value) {
              <div class="flex items-center gap-2">
                ...
                />
                <label tonLabel [for]="'pay-' + opt.value">{{ opt.label }}</label>
              </div>
            }
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="sizesCode">
          <div class="flex items-center gap-6">
            <div class="flex items-center gap-2">
              <input type="radio" tonRadio size="sm" name="size-demo" id="r-sm" checked />
              <label tonLabel for="r-sm">Small</label>
            </div>
            <div class="flex items-center gap-2">
              <input type="radio" tonRadio size="md" name="size-demo" id="r-md" />
              <label tonLabel for="r-md">Medium</label>
            </div>
            <div class="flex items-center gap-2">
              <input type="radio" tonRadio size="lg" name="size-demo" id="r-lg" />
              <label tonLabel for="r-lg">Large</label>
            </div>
          </div>
        </docs-component-preview>
      </section>
...
export class RadioDocComponent {
  ...
  importCode = `import { TonRadioDirective } from '@tony-ui/core';`;
  basicCode = `<input type="radio" tonRadio name="payment" value="card"
       [checked]="selected() === 'card'"
       (change)="selected.set('card')" />`;
  sizesCode = `<input type="radio" tonRadio size="sm" />
<input type="radio" tonRadio size="md" />
<input type="radio" tonRadio size="lg" />`;
