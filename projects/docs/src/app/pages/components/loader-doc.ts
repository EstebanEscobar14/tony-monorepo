...
import { TonLoaderComponent } from '@tony-ui/core';

@Component({
  selector: 'docs-loader-doc',
  standalone: true,
  imports: [CodeBlockComponent, ComponentPreviewComponent, PropsTableComponent, TonLoaderComponent],
  template: `
    <div class="space-y-8">
      <div>
            ...
            @for (v of ['spinner', 'dots', 'bars']; track v) {
              <div class="flex flex-col items-center gap-3">
                @for (s of ['sm', 'md', 'lg', 'xl']; track s) {
                  <ton-loader [variant]="$any(v)" [size]="$any(s)" />
                }
              </div>
            }
          </div>
...
export class LoaderDocComponent {
  ...
  importCode = `import { TonLoaderComponent } from '@tony-ui/core';`;
  variantsCode = `<ton-loader variant="spinner" />
<ton-loader variant="dots" />
<ton-loader variant="bars" />`;
  sizesCode = `<ton-loader size="sm" />
<ton-loader size="md" />
<ton-loader size="lg" />
<ton-loader size="xl" />`;
  exampleCode = `<!-- All variants and sizes -->
<ton-loader variant="spinner" size="lg" />
<ton-loader variant="dots" size="md" />
<ton-loader variant="bars" size="xl" />`;
