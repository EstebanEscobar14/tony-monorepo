...
import { TonButtonDirective, TonToastService, TonToasterComponent } from '@tony-ui/core';
...
@Component({
  ...
  standalone: true,
  imports: [CodeBlockComponent, ComponentPreviewComponent, PropsTableComponent, TonButtonDirective, TonToasterComponent],
  template: `
    <div class="space-y-8">
      <div>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="variantsCode">
          <button tonBtn (click)="showDefault()">Default</button>
          <button tonBtn variant="secondary" (click)="showSuccess()">Success</button>
          <button tonBtn variant="outline" (click)="showWarning()">Warning</button>
          <button tonBtn variant="destructive" (click)="showError()">Error</button>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="actionCode">
          <button tonBtn variant="outline" (click)="showWithAction()">Show with Action</button>
        </docs-component-preview>
      </section>
        ...
        <docs-component-preview [code]="asyncFeedbackCode" language="typescript">
          <div class="flex flex-col gap-4 items-start">
            <div class="flex gap-3">
              <button tonBtn [loading]="saving()" (click)="saveToApi()">
                {{ saving() ? 'Saving...' : 'Save to API' }}
              </button>
              <button tonBtn variant="outline" (click)="deleteWithUndo()">Delete Item</button>
            </div>
      ...
      <ton-toaster position="bottom-right" />
    </div>
  `,
})
export class ToastDocComponent {
  ...
  importCode = `import { TonToastService, TonToasterComponent } from '@tony-ui/core';`;
...
toastService.error('Error', 'Something went wrong.');`;
  ...
  imports: [TonButtonDirective, TonToasterComponent],
  template: \`
    <div class="flex flex-col gap-4 items-start">
      <div class="flex gap-3">
        <button tonBtn [loading]="saving()" (click)="saveToApi()">
