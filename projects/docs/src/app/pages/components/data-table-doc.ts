    ...
    <div class="space-y-8">
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="customCellCode" language="markup">
          <ton-data-table
            ...
            [filterable]="false"
          >
            <ng-template tonCell="status" let-value>
              @let badgeVariant = value === 'Active' ? 'default' : value === 'Pending' ? 'secondary' : 'destructive';
              <span tonBadge [variant]="badgeVariant">{{ value }}</span>
            </ng-template>
            <ng-template tonCell="actions" let-row="row">
              <button tonBtn variant="ghost" size="sm">Edit</button>
            </ng-template>
          </ton-data-table>
        </docs-component-preview>
      </section>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="loadingCode" language="markup">
          <div class="space-y-2">
            <button tonBtn variant="outline" size="sm" (click)="loadingDemo.set(!loadingDemo())">
              Toggle Loading
            </button>
            <ton-data-table
              [columns]="basicColumns"
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="bulkCode" language="markup">
          <ton-data-table
            ...
            [trackBy]="'id'"
          >
            <ng-template tonBulkActions let-selected>
              <button tonBtn variant="destructive" size="sm">Delete ({{ selected.length }})</button>
              <button tonBtn variant="outline" size="sm">Archive</button>
            </ng-template>
          </ton-data-table>
        </docs-component-preview>
      </section>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="expandCode" language="markup">
          <ton-data-table
            ...
            [trackBy]="'id'"
          >
            <ng-template tonRowExpand let-row>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="fullCode" language="markup">
          ...
          >
            <ng-template tonCell="role" let-value>
              <span tonBadge [variant]="value === 'Admin' ? 'default' : 'secondary'">{{ value }}</span>
            </ng-template>
            <ng-template tonBulkActions let-selected>
              <button tonBtn variant="destructive" size="sm">Delete ({{ selected.length }})</button>
            </ng-template>
...
<ton-data-table [columns]="columns" [data]="data" />`;

  filterCode = `<ton-data-table
  [columns]="columns"
  [data]="data"
  [filterable]="true"
/>`;

  customCellCode = `<ton-data-table [columns]="columns" [data]="data">
  <ng-template tonCell="status" let-value>
    <span tonBadge [variant]="value === 'Active' ? 'default' : 'destructive'">
      {{ value }}
    </span>
  </ng-template>
  <ng-template tonCell="actions" let-row="row">
...
/>`;
  ...
  bulkCode = `<ton-data-table [columns]="columns" [data]="data" [selectable]="true">
  <ng-template tonBulkActions let-selected>
    <button tonBtn variant="destructive" size="sm">
  ...
  expandCode = `<ton-data-table [columns]="columns" [data]="data" [expandable]="true" [trackBy]="'id'">
  <ng-template tonRowExpand let-row>
    <div class="p-4">
  ...
  columnToggleCode = `<ton-data-table
  [columns]="columns"
  [data]="data"
  [showColumnToggle]="true"
/>`;
