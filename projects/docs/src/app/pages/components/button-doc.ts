...
import { TonButtonDirective, TonLoaderComponent, type ButtonVariant, type ButtonSize } from '@tony-ui/core';
...
@Component({
  ...
  standalone: true,
  imports: [CodeBlockComponent, ComponentPreviewComponent, PropsTableComponent, TonButtonDirective, TonLoaderComponent],
  template: `
    <div class="space-y-8">
      <div>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="variantsCode">
          @for (v of variants; track v) {
            <button tonBtn [variant]="v">{{ v }}</button>
          }
        </docs-component-preview>
      </section>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="statesCode">
          <button tonBtn [disabled]="true">Disabled</button>
          <button tonBtn [loading]="true">
            <ton-loader variant="spinner" size="sm" />
            Loading
          </button>
          <button tonBtn [loading]="true">
            <ton-loader variant="dots" size="sm" />
            Loading dots
          </button>
        </docs-component-preview>
      </section>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="linkCode">
          <a tonBtn variant="link" href="#">Link Button</a>
          <a tonBtn variant="outline" href="#">Outline Link</a>
        </docs-component-preview>
      </section>
      ...
      <section class="space-y-4">
        ...
        <docs-component-preview [code]="asyncButtonCode" language="typescript">
          <div class="flex items-center gap-4">
            <button tonBtn [loading]="saving()" [disabled]="saving()" (click)="handleSave()">
              {{ saving() ? 'Saving...' : saved() ? 'Saved!' : 'Save Changes' }}
            </button>
            <button tonBtn variant="outline" [disabled]="saving()" (click)="reset()">Reset</button>
          </div>
        </docs-component-preview>
      </section>
...
export class ButtonDocComponent {
  ...
  basicCode = `<button tonBtn>Click me</button>`;

  variantsCode = `<button tonBtn>Default</button>
<button tonBtn variant="destructive">Destructive</button>
<button tonBtn variant="outline">Outline</button>
<button tonBtn variant="secondary">Secondary</button>
<button tonBtn variant="ghost">Ghost</button>
<button tonBtn variant="link">Link</button>`;

  sizesCode = `<button tonBtn size="sm">Small</button>
<button tonBtn size="md">Medium</button>
<button tonBtn size="lg">Large</button>
<button tonBtn size="icon">...</button>`;

  statesCode = `<button tonBtn [disabled]="true">Disabled</button>

<button tonBtn [loading]="isLoading()">
  @if (isLoading()) { <ton-loader variant="spinner" size="sm" /> }
  Save
</button>`;

  linkCode = `<a tonBtn variant="link" href="#">Link Button</a>
<a tonBtn variant="outline" href="#">Outline Link</a>`;
