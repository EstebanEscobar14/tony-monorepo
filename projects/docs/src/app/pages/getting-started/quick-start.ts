...
import { TonButtonDirective } from '@tony-ui/core';
...
@Component({
  ...
  standalone: true,
  imports: [CodeBlockComponent, ComponentPreviewComponent, TonButtonDirective],
  template: `
    <div class="space-y-8">
      <div>
...
@Component({
  ...
  template: \`<button tonBtn>Click me</button>\`,
})
export class ExampleComponent {}`;

  usageCode = `<button tonBtn>Click me</button>
<button tonBtn variant="outline">Outline</button>
<button tonBtn variant="secondary">Secondary</button>`;

  inputsCode = `<button tonBtn variant="destructive" size="lg">Delete</button>
<button tonBtn [disabled]="true">Disabled</button>
<button tonBtn [loading]="isLoading">Submit</button>`;
}
