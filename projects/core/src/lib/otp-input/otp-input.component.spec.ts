...
import { TonOtpInputComponent } from './otp-input.component';

@Component({
  standalone: true,
  imports: [TonOtpInputComponent],
  template: `
    <ton-otp-input
      [(value)]="otp"
      [length]="length()"
...
@Component({
  standalone: true,
  imports: [ReactiveFormsModule, TonOtpInputComponent],
  template: `<ton-otp-input [formControl]="ctrl" [autoFocus]="false" />`,
})
class ReactiveFormHost {
  ctrl = new FormControl('');
}

describe('TonOtpInputComponent — Reactive Forms', () => {
