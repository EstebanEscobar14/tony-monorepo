...
import { TonIndicatorDirective, TonIndicatorBadgeDirective, type IndicatorPosition, type IndicatorVariant } from './indicator.directives';

@Component({
  standalone: true,
  imports: [TonIndicatorDirective, TonIndicatorBadgeDirective],
  template: `
    <div tonIndicator>
      <span tonIndicatorBadge [position]="position()" [variant]="variant()" [ariaLabel]="badgeLabel()">5</span>
      <div>Content</div>
    </div>
  `,
})
...
describe('TonIndicatorDirective', () => {
  ...
  it('should render indicator container', () => {
    const el = fixture.nativeElement.querySelector('[tonIndicator]');
    expect(el.className).toContain('relative');
    expect(el.className).toContain('inline-flex');
  });

  it('should position badge at top-end by default', () => {
    const badge = fixture.nativeElement.querySelector('[tonIndicatorBadge]');
