...
import { TonRatingComponent } from './rating.component';
import type { RatingSize } from './rating.variants';

@Component({
  standalone: true,
  imports: [TonRatingComponent],
  template: `
    <ton-rating
      [(value)]="value"
      [max]="max()"
...
class TestHostComponent {
  ...
  half = signal(false);
  rating = viewChild(TonRatingComponent);
}

describe('TonRatingComponent', () => {
  ...
  beforeEach(async () => {
    ...
    host = fixture.nativeElement.querySelector('ton-rating');
  });
...
@Component({
  standalone: true,
  imports: [ReactiveFormsModule, TonRatingComponent],
  template: `<ton-rating [formControl]="ctrl" />`,
})
class ReactiveFormHost {
  ctrl = new FormControl(0);
}

describe('TonRatingComponent — Reactive Forms', () => {
