  ...
  template: `
    <div class="relative w-full" [style.aspect-ratio]="'16/9'">
      <div class="absolute inset-0">
        <ng-content select="[tonDiffAfter]" />
      </div>
      <div class="absolute inset-0 overflow-hidden" [style.width.%]="value()">
        <ng-content select="[tonDiffBefore]" />
      </div>
      <div
...
export class TonDiffComponent {
  ...
  private updateValue(event: PointerEvent): void {
    const target = (event.currentTarget as HTMLElement).closest('ton-diff');
    if (!target) return;
