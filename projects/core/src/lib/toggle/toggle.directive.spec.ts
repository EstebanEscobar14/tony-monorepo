...
import { TonToggleDirective } from './toggle.directive';
import type { ToggleVariant, ToggleSize } from './toggle.variants';

@Component({
  standalone: true,
  imports: [TonToggleDirective],
  template: `<button tonToggle [variant]="variant()" [size]="size()" [(pressed)]="pressed">Toggle</button>`,
})
...
@Component({
  standalone: true,
  imports: [ReactiveFormsModule, TonToggleDirective],
  template: `<button tonToggle [formControl]="ctrl">Toggle</button>`,
})
class ReactiveFormHost {
  ctrl = new FormControl(false);
}

describe('TonToggleDirective — Reactive Forms', () => {
