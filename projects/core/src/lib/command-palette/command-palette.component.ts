...
import { TON_DIALOG_DATA } from '../modal/dialog.service';
import type { Command, CommandGroup, CommandPaletteConfig } from './command-palette.types';

@Component({
  selector: 'ton-command-palette',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [TonInputDirective],
  host: {
  ...
  styles: `
    :host {
      display: block;
      background-color: var(--ton-background);
  ...
  template: `
    ...
    <!-- Results -->
    <div class="max-h-[300px] overflow-y-auto p-2 ton-scrollbar">
...
export class TonCommandPaletteComponent {
  readonly config = inject<CommandPaletteConfig>(TON_DIALOG_DATA);
