...
import { TonColorPickerComponent } from './color-picker.component';
...
describe('TonColorPickerComponent', () => {
  ...
  it('should close popover on escape', () => {
    ...
    const host = el.querySelector('ton-color-picker') as HTMLElement;
...
@Component({
  standalone: true,
  imports: [ReactiveFormsModule, TonColorPickerComponent],
  template: `<ton-color-picker [formControl]="ctrl" [inline]="true" />`,
})
class ReactiveFormHost {
  ctrl = new FormControl('#ff0000');
}

describe('TonColorPickerComponent — Reactive Forms', () => {
