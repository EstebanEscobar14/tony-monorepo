...
import { TonTagInputComponent } from './tag-input.component';

@Component({
  standalone: true,
  imports: [TonTagInputComponent],
  template: `
    <ton-tag-input
      [(value)]="tags"
      [maxTags]="maxTags()"
...
@Component({
  standalone: true,
  imports: [ReactiveFormsModule, TonTagInputComponent],
  template: `<ton-tag-input [formControl]="ctrl" />`,
})
class ReactiveHost {
  ctrl = new FormControl<string[]>([]);
}

describe('TonTagInputComponent — Reactive Forms', () => {
