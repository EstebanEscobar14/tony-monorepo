...
import { TonDatePickerComponent } from './date-picker.component';

@Component({
  standalone: true,
  imports: [TonDatePickerComponent],
  template: `
    <ton-date-picker
      [(value)]="selectedDate"
...
describe('TonDatePickerComponent', () => {
  ...
  it('should open dropdown on click', () => {
    ...
    trigger.click();
    fixture.detectChanges();
    const calendar = el.querySelector('ton-calendar');
  ...
  it('should close dropdown when date is selected', () => {
    ...
    const dayBtn = el.querySelectorAll('ton-calendar [role="grid"] button:not([disabled])');
...
@Component({
  standalone: true,
  imports: [ReactiveFormsModule, TonDatePickerComponent],
  template: `<ton-date-picker [formControl]="ctrl" />`,
})
class ReactiveFormHost {
  ctrl = new FormControl<Date | null>(null);
}

describe('TonDatePickerComponent — Reactive Forms', () => {
