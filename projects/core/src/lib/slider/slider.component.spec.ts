...
import { TonSliderComponent } from './slider.component';

@Component({
  standalone: true,
  imports: [TonSliderComponent],
  template: `<ton-slider [(value)]="value" [min]="min()" [max]="max()" [step]="step()" [disabled]="disabled()" />`,
})
...
describe('TonSliderComponent', () => {
  ...
  it('should increment with ArrowRight', () => {
    const host = fixture.nativeElement.querySelector('ton-slider');
...
@Component({
  standalone: true,
  imports: [ReactiveFormsModule, TonSliderComponent],
  template: `<ton-slider [formControl]="ctrl" />`,
})
class ReactiveFormHost {
  ctrl = new FormControl(50);
}

describe('TonSliderComponent — Reactive Forms', () => {
