...
import { TonDataTableComponent } from './data-table.component';
import {
  TonCellDefDirective,
...
@Component({
  ...
  template: `
    <ton-data-table
      ...
      [(selectedRows)]="selectedRows"
    >
      <ng-template tonCell="name" let-value let-row="row">
        <span class="custom-cell">{{ value }}</span>
      </ng-template>
      <ng-template tonHeaderCell="name" let-col>
...
describe('TonDataTableComponent', () => {
  ...
  it('should filter rows by text', () => {
    fixture.detectChanges();
    const input = el.querySelector('input[toninput]') as HTMLInputElement;
    input.value = 'User 01';
  ...
  it('should navigate pagination', () => {
    ...
    const nextBtn = el.querySelector('ton-pagination button:last-child') as HTMLButtonElement;
    nextBtn.click();
    fixture.detectChanges();
...
describe('TonDataTableComponent (enhanced)', () => {
  ...
  it('should show column toggle when enabled', () => {
    ...
    const toggleBtn = el.querySelector('[tondropdowntrigger]');
    expect(toggleBtn).toBeTruthy();
  });

  it('should keep the column toggle menu aligned without forcing right overflow classes', () => {
    host.showColumnToggle.set(true);
    fixture.detectChanges();

    const content = el.querySelector('[tondropdowncontent]') as HTMLElement;
    ...
    expect(content.className).not.toContain('left-auto');
  });
});
