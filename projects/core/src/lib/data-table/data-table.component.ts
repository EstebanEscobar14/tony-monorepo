        ...
        <div class="flex items-center gap-2">
          ...
          @if (showColumnToggle()) {
            <div tonDropdown class="relative">
              ...
              <div tonDropdownContent class="w-48 max-w-[calc(100vw-2rem)]">
      ...
      <div class="border border-border rounded-sm overflow-hidden">
        ...
        >
          ...
          @for (col of visibleColumns(); track col.key) {
            <button
              type="button"
              class="px-3 py-2 text-left min-w-0"
        ...
        >
          ...
          <div
            *cdkVirtualFor="let row of paginatedData(); trackBy: cdkTrackBy"
            class="grid border-b border-border text-sm hover:bg-muted/40"
            [style.grid-template-columns]="gridTemplateColumns()"
            [style.min-height.px]="virtualItemSize()"
            (click)="onRowClick(row)"
          >
            @for (col of cols; track col.key) {
              <div class="px-3 py-2 min-w-0 flex items-center" [style.width]="col.width ?? null">
...
export class TonDataTableComponent {
  ...
  readonly virtualItemSize = input(52);
  readonly virtualViewportHeight = input('520px');
  ...
  readonly gridTemplateColumns = computed(() =>
    this.visibleColumns()
      .map((col) => col.width ?? 'minmax(10rem, 1fr)')
      .join(' ')
  );
  ...
  cdkTrackBy = (index: number, row: Record<string, unknown>): unknown =>
    this.trackByFn(row, index);
