...
import { TonDateRangePickerComponent } from './date-range-picker.component';
import type { DateRange, DatePickerPreset } from '../calendar/calendar.types';

@Component({
  standalone: true,
  imports: [TonDateRangePickerComponent],
  template: `
...
describe('TonDateRangePickerComponent', () => {
  ...
  it('should open dropdown with single calendar by default', () => {
    ...
    trigger.click();
    fixture.detectChanges();
    const calendars = el.querySelectorAll('ton-calendar');
...
@Component({
  standalone: true,
  imports: [ReactiveFormsModule, TonDateRangePickerComponent],
  template: `<ton-date-range-picker [formControl]="ctrl" />`,
})
class ReactiveFormHost {
  ctrl = new FormControl<DateRange | null>(null);
}

describe('TonDateRangePickerComponent — Reactive Forms', () => {
