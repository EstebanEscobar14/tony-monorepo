...
export function ngAdd(options: { project?: string; theme?: string }): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    if (tree.exists(stylesPath)) {
      ...
      const themeImport = '@import "@tony-ui/core/styles/sonny-theme.css";';
      const sourceDirective = '@source "../node_modules/@tony-ui/core";';
    ...
    if (tree.exists(configPath)) {
      ...
      if (!configContent.includes('provideTonyUI')) {
        ...
        const importLine = `import { provideTonyUI } from '@tony-ui/core';\n`;
