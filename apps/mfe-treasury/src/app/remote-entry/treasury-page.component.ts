...
export class TreasuryPageComponent {
  ...
  readonly mobileVisibleCount = signal(12);
  readonly mobilePreviewRows = computed(() => this.rows().slice(0, this.mobileVisibleCount()));
  readonly canLoadMoreMobile = computed(() => this.mobileVisibleCount() < this.rows().length);
  ...
  showMoreMobileRows(): void {
    this.mobileVisibleCount.update((count) => Math.min(count + 12, this.rows().length));
  }
}
